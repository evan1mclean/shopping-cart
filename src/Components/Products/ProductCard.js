  ...
  return (
    <div className="flex flex-col bg-white py-5 gap-4 border-2 rounded-lg">
      ...
      <div className="flex-1 flex flex-col gap-2 border-t-2 px-5 pt-5">
        ...
        <p className="mt-auto">${price.toFixed(2)}</p>
        <button className="bg-sky-400 hover:bg-sky-500 active:bg-sky-400 text-white py-2 px-4" onClick={() => addToCart(product)}>Add To Cart</button>
      </div>
    </div>
  );
}